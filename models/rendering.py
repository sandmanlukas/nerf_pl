    ...
    def inference(results, model, xyz, z_vals, test_time=False, **kwargs):
        ...
        if typ == "coarse" and test_time:
            ...
            for i in range(0, B, chunk):
                ...
                out_chunks += [
                    model(torch.cat(inputs, 1), output_transient=output_transient)
                ]
        ...
        if output_transient:
            ...
            if test_time:
                ...
                if white_back:
                    static_rgb_map_ += 0 - rearrange(weights_sum, "n -> n 1")
    ...
    if N_importance > 0:  # sample points for fine model
        ...
        if model.encode_appearance:
            if "a_embedded" in kwargs:
                ...
                a_embedded = embeddings["a"](ts)
        output_transient = (
            kwargs.get("output_transient", True) and model.encode_transient
        )
        if output_transient:
